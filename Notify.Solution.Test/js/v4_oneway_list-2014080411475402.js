...
trackAction = function() { $jex.console.trace("CALL OLD API __ trackAction"); };
var TsinghuaOneWayTracker = (function() {
    ...
    b.traceReAndRfList = function(e) {
        ...
        $jex.foreach(e, function(r) {
            ...
            var q = [0, 0];
            if (!o) {
                var u = r.sinfoCache, n = ["hot", "ps", "late", "lcc"];
                if (u) {
                    for (var l = 0; l < 4; l++) {
                        if (u[n[l]]) {
                            q[0] = l + 1;
    ...
    b.trackWrappers = function(j) {
        ...
        var h = j.code();
        var r = j.codeShare(), e = j.codeShareFlight();
        if (r && e) {
            if (h !== r) {
                h += ">" + r;
            }
...
(function(a) {
    ...
    BOOMR.version = "0.9";
    e = {
        beacon_url: "", site_domain: a.location.hostname.replace(/.*?([^.]+\.[^.]+)\.?$/, "$1").toLowerCase(), user_ip: "", events: { page_ready: [], page_unload: [], visibility_changed: [], before_beacon: [] }, vars: {}, disabled_plugins: {},
    ...
    c = {
        utils: {
            ...
            removeCookie: function(d) { return this.setCookie(d, {}, 0, "/", null); },
        ...
        init: function(h) {
            ...
            if (typeof h.autorun === "undefined" || h.autorun !== false) {
                e.addListener(a, "load", function() { e.fireEvent("page_ready"); });
            }
            e.addListener(g, "webkitvisibilitychange", function() { e.fireEvent("visibility_changed"); });
            e.addListener(a, "unload", function() { a = null; });
            return this;
        },
        page_ready: function() {
    ...
    if (a.YAHOO && a.YAHOO.widget && a.YAHOO.widget.Logger) {
        ...
        if (typeof a.Y !== "undefined" && typeof a.Y.log !== "undefined") {
            ...
            if (typeof console !== "undefined" && typeof console.log !== "undefined") {
                c.log = function(d, h, i) { console.log(i + ": [" + h + "] ", d); };
            }
        }
    }
    for (b in c) {
...
(function(a) {
    ...
    BOOMR.plugins = BOOMR.plugins || {};
    var b = {
        complete: false, timers: {}, cookie: "RT", cookie_exp: 600, strict_referrer: true, navigationStart: undefined, responseStart: undefined,
        start: function() {
            var e, d = new Date().getTime();
            if (!this.cookie) {
                return this;
            }
            if (!BOOMR.utils.setCookie(this.cookie, { s: d, r: c.URL.replace(/#.*/, "") }, this.cookie_exp, "/", null)) {
        ...
        initNavTiming: function() {
            ...
            if (e && e.timing) {
                ...
                if (a.chrome && a.chrome.csi) {
                    d = { navigationStart: a.chrome.csi().startE, responseStart: undefined };
                    BOOMR.addVar("rt.start", "csi");
                } else {
                    if (a.gtbExternal) {
                        d = { navigationStart: a.gtbExternal.startE(), responseStart: undefined };
    ...
    BOOMR.plugins.RT = {
        ...
        setTimer: function(d, e) {
            ...
            var j, d, h, e, i = { t_done: 1, t_resp: 1, t_page: 1 }, g = 0, k, f, l = [];
            if (b.complete) {
                return this;
            }
            b.initNavTiming();
...
(function(b) {
    ...
    BOOMR = BOOMR || {};
    BOOMR.plugins = BOOMR.plugins || {};
    var a = [{ name: "image-0.png", size: 11483, timeout: 1400 }, { name: "image-1.png", size: 40658, timeout: 1200 }, { name: "image-2.png", size: 164897, timeout: 1300 }, { name: "image-3.png", size: 381756, timeout: 1500 }, { name: "image-4.png", size: 1234664, timeout: 1200 }, { name: "image-5.png", size: 4509613, timeout: 1200 }, { name: "image-6.png", size: 9084559, timeout: 1200 }];
    a.end = a.length;
    a.start = 0;
    a.l = { name: "image-l.gif", size: 35, timeout: 1000 };
    var c = {
        base_url: "images/", timeout: 15000, nruns: 5, latency_runs: 10, user_ip: "", cookie_exp: 7 * 86400, cookie: "BA", results: [], latencies: [], latency: null, runs_left: 0, aborted: false, complete: false, running: false, ncmp: function(f, d) { return (f - d); },
        iqr: function(h) {
            var g = h.length - 1, f, m, k, d = [], j;
        ...
        calc_latency: function() {
            ...
            return { mean: k, median: m, stddev: d, stderr: o };
        },
        calc_bw: function() {
            var y, x, t = 0, p, g = [], v = [], f = 0, o = 0, C = 0, u = 0, q, A, B, h, d, w, k, m, l, z, s;
            ...
            return { mean: q, stddev: A, stderr: B, median: h, mean_corrected: d, stddev_corrected: w, stderr_corrected: k, median_corrected: m };
        },
        defer: function(f) {
            var d = this;
        ...
        load_img: function(g, k, m) {
            var f = this.base_url + a[g].name + "?t=" + (new Date().getTime()) + Math.random(), l = 0, j = 0, d = new Image(), h = this;
            d.onload = function() {
        ...
        img_loaded: function(f, g, h, j) {
            ...
            if (j === null) {
                this.results[this.nruns - h].r[f + 1] = { t: null, state: null, run: h };
                return;
            }
            var d = { start: g, end: new Date().getTime(), t: null, state: j, run: h };
            if (j) {
                d.t = d.end - d.start;
            }
        ...
        finish: function() {
            ...
            var f = this.calc_bw(), d = { bw: f.median_corrected, bw_err: parseFloat(f.stderr_corrected, 10), lat: this.latency.mean, lat_err: parseFloat(this.latency.stderr, 10), bw_time: Math.round(new Date().getTime() / 1000) };
            BOOMR.addVar(d);
            if (!isNaN(d.bw)) {
                BOOMR.utils.setCookie(this.cookie, { ba: Math.round(d.bw), be: d.bw_err, l: d.lat, le: d.lat_err, ip: this.user_ip, t: d.bw_time }, (this.user_ip ? this.cookie_exp : 0), "/", null);
            }
            this.complete = true;
            BOOMR.sendBeacon();
            this.running = false;
        ...
        setVarsFromCookie: function(l) {
            var i = parseInt(l.ba, 10), k = parseFloat(l.be, 10), j = parseInt(l.l, 10) || 0, f = parseFloat(l.le, 10) || 0, d = l.ip.replace(/\.\d+$/, "0"), m = parseInt(l.t, 10), h = this.user_ip.replace(/\.\d+$/, "0"), g = Math.round((new Date().getTime()) / 1000);
            if (d === h && m >= g - this.cookie_exp) {
                this.complete = true;
                BOOMR.addVar({ bw: i, lat: j, bw_err: k, lat_err: f });
                return true;
            }
            return false;
        }
    ...
    BOOMR.plugins.BW = {
        ...
        is_complete: function() { return c.complete; }
    };
}(window));
...
CACTI_monitoring.prototype._init = function() {
    ...
    var a = this;
    this.isStart = true;
    BOOMR.init({ beacon_url: this._url, autorun: false, RT: { enabled: true, strict_referrer: true }, BW: { enabled: false } }).addVar("page_id", this._pageId).subscribe("before_beacon", function(d) {
        var b = [], c;
        if ("t_done" in d) {
            return;
        }
    ...
    $jex.event.bind(window, "load", function() { a.end("t_done"); });
};
...
CACTI_monitoring.prototype.send = function() { BOOMR.page_ready(); };
...
CommonInfoManager.prototype.setDataLoad = function(a) { this._dataStat = a; };
CommonInfoManager.prototype.getDataLoad = function() { return this._dataStat; };
CommonInfoManager.prototype.addAirportSource = function(b, a) { this.addSource("airport", b, a); };
CommonInfoManager.prototype.addVendorSource = function(b, a) { this.addSource("vendor", b, a); };
CommonInfoManager.prototype.addNotWorkVendors = function(b, a) { this.addSource("notWork", b, a); };
CommonInfoManager.prototype.addSuperOTAMaxNum = function(b, a) { this.addSource("maxSuper", b, a); };
CommonInfoManager.prototype.addCarrierSource = function(b, a) { this.addSource("carrier", b, a); };
CommonInfoManager.prototype.addCitySource = function(b, a) { this.addSource("city", b, a); };
CommonInfoManager.prototype.addPlaneSource = function(b, a) { this.addSource("plane", b, a); };
CommonInfoManager.prototype.addFlightLineVendorSource = function(b, a) { this.addSource("flightLineVendor", b, a); };
CommonInfoManager.prototype.addOriginalPrice = function(b, a) { this.addSource("oprice", b, a); };
CommonInfoManager.prototype.addInsuranceSum = function(b, a) { this.addSource("insurancesum", b, a); };
var FlightInfoManager = function() { FlightInfoManager.superclass.constructor.call(this); };
$jex.extendClass(FlightInfoManager, InfoManager);
FlightInfoManager.prototype.addFlightInfoSource = function(b, a) { this.addSource("flightInfo", b, a); };
...
FlightInfoManager.prototype.addFlightInfoItem = function(b, c, a) { this.addItem("flightInfo", b, c, a); };
FlightInfoManager.prototype.addCorrSource = function(b, a) { this.addSource("corrInfo", b, a); };
FlightInfoManager.prototype.addExtInfoSource = function(b, a) { this.addSource("extInfo", b, a); };
FlightInfoManager.prototype.replacePriceData = function(c, b) {
    b = this._getPriceType(b);
    var a = this.get(b);
    $jex.foreach(c, function(e, d, f) { a[f] = e; });
};
FlightInfoManager.prototype._getPriceType = function(a) { return (!a || a == "all") ? "my_wrappInfo" : ("my_wrappInfo_" + a); };
...
FlightInfoManager.prototype.addPriceDataItem = function(b, c, a) { this.addItem("priceData", b, c, a); };
...
FlightInfoManager.prototype.addSpecialWrapper = function(b, a) { this.addSource("PayCarrier", b, a); };
FlightInfoManager.prototype.addPriceInfoSource = function(b, a) { this.addSource("priceInfo", b, a); };
FlightInfoManager.prototype.addZYFAirlines = function(b, c, a) { this.addItem("ZYFAirlines", b, c, a); };
FlightInfoManager.prototype.addZYFReference = function(b, c, a) { this.addItem("ZYFReference", b, c, a); };
FlightInfoManager.prototype.getZYFAirlines = function(a) { return this.get("ZYFAirlines", a); };
FlightInfoManager.prototype.getZYFReference = function(a) { return this.get("ZYFReference", a); };
var UICacheManager = (function() {
    var a = function() { FlightInfoManager.superclass.constructor.call(this); };
    $jex.extendClass(a, InfoManager);
    a.prototype.addToCache = function(b) { this.addItem("uiCache", b.newid(""), b); };
    a.prototype.getCache = function(b) { return this.get("uiCache", b); };
    return new a();
})();
...
var DomesticOnewayDataAnalyzer = new (function() {
    ...
    this._initial = function() {
        f = new DataSet({
            ...
            filterFunc: {
                "航空公司": function(K, I, J) { return [K.carrierCode()]; }, "起飞时间": function(I) { return [I.deptTimeRangeValue()]; }, "机型": function(I) { return I.planeType(); }, "起飞机场": function(I) { return I.airportCodes(); }, "降落机场": function(I) { return I.airportCodes(); },
                "方式": function(I) {
                    if (I.type == "compose") {
                        return "transfer";
                    }
                ...
                "中转城市": function(I) { return I.transferCity(); }
            },
            pageSize: 30
        });
        if (typeof FlightListUISorter != "undefined") {
            $jex.event.binding(f, "refreshCurrentPage", function(K, L, I, J) { FlightListUISorter.resort(K, L, I, J); });
        }
    };
    var m = function() {
        $jex.event.trigger(t, "updateFilter", { catalog: "航空公司", name: this.carrier().zh, value: this.flightInfo().ca });
        var K = this.deptTimeRange();
        $jex.event.trigger(t, "updateFilter", { catalog: "起飞时间", name: K.zh, value: K.value });
        var L = this.plane();
        $jex.foreach(L.type, function(M) { $jex.event.trigger(t, "updateFilter", { catalog: "机型", name: M, value: M }); });
        var I = this.deptAirport();
        $jex.event.trigger(t, "updateFilter", { catalog: "起飞机场", group: this.deptCityCode(), name: I.ab, value: I.key || I.code });
        var J = this.arriAirport();
        $jex.event.trigger(t, "updateFilter", { catalog: "降落机场", group: this.arriCityCode(), name: J.ab, value: J.key || J.code });
    };
    var q = function() {
        var K = this.firstTrip();
        if (this.carrierCode()) {
            $jex.event.trigger(t, "updateFilter", { catalog: "航空公司", name: K.carrier().zh, value: K.flightInfo().ca });
        }
        var I = K.deptTimeRange();
        $jex.event.trigger(t, "updateFilter", { catalog: "起飞时间", name: I.zh, value: I.value });
        var J = K.plane();
        $jex.foreach(J.type, function(M) { $jex.event.trigger(t, "updateFilter", { catalog: "机型", name: M, value: M }); });
        K = this.secondTrip();
        var J = K.plane();
        $jex.foreach(J.type, function(M) { $jex.event.trigger(t, "updateFilter", { catalog: "机型", name: M, value: M }); });
        var L = K.deptCity();
        $jex.event.trigger(t, "updateFilter", { catalog: "中转城市", name: L.zh, value: L.en });
    };
    this.hasWrapper = function(I) { return this.infoMgr().get("vendor", I); };
    this.resultData = function() { return f.currentPageData(); };
    this.currentPageIndex = function() { return f.currentPage; };
    this.getDataSet = function() { return f; };
    this.getData = function() { return f.getData(); };
    this.pageInfo = function() {
        var I = { pageCount: f.pageCount(), pageSize: f.pageSize(), pageIndex: f.pageIndex() };
        return I;
    };
    ...
    function d(L) {
        ...
        if (N) {
            if (M) {
                $jex.event.trigger(t, "updateFilter", { catalog: "方式", name: "直达", value: "oneway" });
            }
            if (K) {
                $jex.event.trigger(t, "updateFilter", { catalog: "方式", name: "中转联程", value: "transfer" });
            }
    ...
    function g() { window.location.href = "/twell/flight/busy.jsp?ret=" + encodeURIComponent(window.location.href.toString()); }
    ...
    function z(K) {
        ...
        J.addVendorSource(K.vendors, { isOverwrite: false });
    ...
    function o(L) {
        var P = L.flightCode, N = L.labelType;
        var I = t.infoMgr();
    ...
    function s(M, Q, K) {
        ...
        $jex.foreach(M.priceData, function(V, U, W) {
            ...
            $jex.foreach(J, function(X) {
                var Y = X + "_nw";
                V[Y] = { wrid: X, type: "notWork", sortRank: 10000000 };
            });
        });
    }
    ...
    function w(L) {
        ...
        var I = [];
        $jex.array.each(J, function(O) { I.push(String.fromCharCode(O.charCodeAt(0) - 1)); });
        I.reverse();
        L.queryID = M + I.join("");
    }
...
var DomesticOnewaySearchService = new (function() {
    ...
    var l = false;
    var x = null;
    this.longwell = function() { return x || {}; };
    var c = [];
    var j = [];
    var b = null;
    var n = null;
    ...
    var k = null;
    var w = null;
    this.setAnalyzer = function(C) { w = C; };
    var r = false;
    ...
    this.search = function(E) {
        ...
        $jex.merge(this.param, { fromCity: E.searchDepartureAirport, toCity: E.searchArrivalAirport, fromDate: E.searchDepartureTime });
        $jex.merge(this.oparam, { ex_track: E.ex_track, from: E.from });
        ...
        QLib.setCookieForSpider(function() { p._invoke_longwell(); });
        v = 1;
        e = new Date();
        setTimeout(function() { $jex.event.trigger(p, "pastLessSecond"); }, 15000);
    };
    this.queryZYF = function() {
        var C = this;
        var D = o + "/zyf/api/ads.json";
        $jex.ajax(D, { dpt: C.param.fromCity, arr: C.param.toCity, dptDate: C.param.fromDate }, function(E) {
            if (E) {
                C._process_zyf(E);
            }
        });
    ...
    this.queryNext = function() {
        ...
        if (this.getTask()) {
            ...
            setTimeout(function() { p.queryNext(); }, 100);
        } else {
            if (l == false && d != 2) {
                p._invoke_AVData();
            } else {
                if (i != 2 && (d == 2 || (d != 2 && c.length >= 2))) {
                    ...
                    setTimeout(function() { p._invoke_oneway(); }, s);
                }
            }
        }
    };
    ...
    this._invoke_longwell = function() {
        ...
        var F = this.param;
        var D = c;
        var E = { "http://www.travelco.com/searchArrivalAirport": F.toCity, "http://www.travelco.com/searchDepartureAirport": F.fromCity, "http://www.travelco.com/searchDepartureTime": F.fromDate, "http://www.travelco.com/searchReturnTime": F.fromDate, locale: "zh", nextNDays: "0", searchLangs: "zh", searchType: "OneWayFlight", tags: 1, mergeFlag: 0, xd: LONGWELLVERSION };
        k = { departureCity: F.fromCity, arrivalCity: F.toCity, departureDate: F.fromDate, returnDate: F.fromDate, nextNDays: "0", searchType: "OneWayFlight", searchLangs: "zh", locale: "zh" };
    ...
    this._process_zyf = function(C) { $jex.event.trigger(p, "zyfLoaded", C); };
    this._invoke_transfer = function() {
        ...
        $jex.ajax(C, D, function(E) {
            ...
            if (E.needNewSearch == true) {
                ...
                setTimeout(function() { p.queryNext(); }, 3500);
            } else {
                $jex.event.trigger(p, "TransferDataReady");
                if (!$jex.$empty(E.data)) {
    ...
    this.isSearchEnd = function() { return v == 2; };
    this._invoke_ExtInfo = function() {
        $jex.console.info("调用扩展信息及准点率");
        var E = this.param;
        var C = o + "/twell/flight/DynamicFlightInfo.jsp";
        var D = { departureCity: E.fromCity, arrivalCity: E.toCity, departureDate: E.fromDate, fromCity: E.fromCity, toCity: E.toCity };
    ...
    this._onerror = function() { $jex.event.trigger(p, "onerror", arguments); };
    var g = [];
...
var PriceCheckService = (function() {
    ...
    function c(n) {
        ...
        return { queryID: System.service.queryId(), departureCity: o.fromCity, arrivalCity: o.toCity, departureDate: o.fromDate, returnDate: o.fromDate, flightCode: n.split("_")[0], label: n.split("_")[1] };
    }
    ...
    function j(o, n) {
        var p = b(o);
        $jex.ajax(a, p, n, { onerror: function() { n(); } });
    }
...
var TransferFlightEntity = function() {
    ...
    this.carrierCode = function() { return (this.firstTrip().carrierCode() == this.secondTrip().carrierCode()) ? this.firstTrip().carrierCode() : ""; };
    this.deptTimeRange = function() { return this.firstTrip().deptTimeRange(); };
    var d = null;
    ...
    this.lowestPrice = function() { return this.firstTrip().lowestPrice() + this.secondTrip().lowestPrice(); };
};
$jex.extendClass(TransferFlightEntity, FlightEntity);
TransferFlightEntity.prototype.flightKeyCode = function() { return this.firstTrip().flightKeyCode() + "|" + this.secondTrip().flightKeyCode(); };
...
TransferFlightEntity.prototype.isIntervalFlight = function() { return this.firstTrip().flightInfo().dd != this.secondTrip().flightInfo().dd; };
...
var SingleTripFlightEntity = function() {
    ...
    this.wrappers = function() {
        if (typeof d == "undefined" || d == null) {
            ...
            d.ownerFlight(this);
            d.update = function() { this.dataSource(b.flightInfoMgr().get("priceData", b.key())); };
            d.update();
        }
        return d;
    };
...
SingleTripFlightEntity.prototype.codeShareFlight = function() { return this.commInfoMgr().entityManager().get(this.codeShare() + "|" + this.deptDate()); };
...
function SingleTripFlightWrapperListEntity() { SingleTripFlightWrapperListEntity.superclass.constructor.call(this); }

$jex.extendClass(SingleTripFlightWrapperListEntity, WrapperListEntity);
SingleTripFlightWrapperListEntity.prototype.createWrapperEntity = function() { return new SingleTripFlightWrapperEntity(); };
SingleTripFlightWrapperListEntity.prototype.sort = function() {
    var b = this.keys(), a = this;
    b.sort(function(d, c) {
        var f = a.get(d);
        var e = a.get(c);
...
SingleTripFlightWrapperEntity.prototype.rankgrade = function() { return Math.round(this.dataSource().dispRank * 10) / 10 || 0; };
SingleTripFlightWrapperEntity.prototype.ranktitle = function() { return FlightUtil.getGTITLE(this.advalue(), 1, this.dataSource().rankline, 5 - this.dataSource().rankline); };
SingleTripFlightWrapperEntity.prototype.comments = function() { return this.dataSource().comments || []; };
...
SingleTripFlightWrapperEntity.prototype.hasPackageprice = function() { return this.bpr(); };
...
SingleTripFlightWrapperEntity.prototype.afeePrice = function() { return this.bpr() && this.price(); };
SingleTripFlightWrapperEntity.prototype.bprPrice = function() { return this.bpr() || this.price(); };
...
$jex.exec(function() {
    ...
    FlightListUISorter.close = function() { c = null; };
    ...
    FlightListUISorter.resort = function(B, G, g, E) {
        ...
        B.sort(function(k, i) { return Q(k) - Q(i); });
        var o;
        var v;
        var r;
        var x = [];
        ...
        $jex.foreach(u, function(i) {
            if (i.length > 0) {
                i.sort(function(n, k) { return z(n) - z(k); });
                f.push(i[0]);
                i.splice(0, 1);
                O = O.concat(i);
            }
        ...
        I = I.concat(O);
        I.sort(function(k, i) { return z(k) - z(i); });
        a(I, G, true);
        I = I.concat(R);
        I = I.concat(D);
        var F = [];
        ...
        H.sort(function(k, i) { return z(k) - z(i); });
        a(H, G, true);
        F = F.concat(H, s);
        m = m.concat(F, I, x);
        B.splice(0, B.length);
        $jex.foreach(m, function(i) { B.push(i); });
...
PagerUI.prototype.go = function(a) { $jex.event.trigger(this, "changePage", a); };
PagerUI.prototype.update = function(f) {
    ...
    b.renderPrefixDot = function() { a.text("..."); };
    b.renderSuffixDot = function() { a.text("..."); };
    b.render();
...
function pageCreator(a, d, b) {
    ...
    this._renderPage = function(g) { this.renderPage(g, g == c); };
...
SearchStatusbar.prototype._init = function() {
    ...
    this.singleNum = 0;
    this.pkgNum = 0;
    $jex.event.binding(a, "loadedFirstData", function() { b.start(); });
    ...
    $jex.event.binding(a, "searchEnd", function() { b.stop(); });
};
SearchStatusbar.prototype.start = function() {
    var a = this;
    clearInterval(this.handler);
    this.handler = setInterval(function() { a.updateStatus(); }, 500);
    this.updateStatus();
};
...
(function() {
    ...
    function a(f, e, h, g) { return h * f / g + e; }

    $jex.ui.lockScreenProgress = function(e, h) {
        var f = e && e.msg || "此次报价已过期，正在重新搜索", g = e && e.time || 3000;
...
TransferFlightVendorListUI.prototype.update = function(a) {
    ...
    this.text("</div>");
    var f = false, d = null;
    var c = { id: "transfer_p1", zIndex: 3, goid: "gotoFirstDetail", msg: "一" };
    d = a.firstTrip();
...
TransPackageFlightWrapperUI.prototype.insert_BOOKING_BUTTON = function(a) { this._buttonHTML("bpr", a); };
$jex.register("TransPackageFlightWrapperUI", TransPackageFlightWrapperUI);

function TransferFlightWrapperListUI(a) { TransferFlightWrapperListUI.superclass.constructor.call(this, a); }
...
var RoundTripFlightRecommend = (new function(a) {
    ...
    this.loadData = function() {
        var b = new ScriptRequest({ funcName: "RoundTripFlightRecommend.callBack", callbackName: "callback" });
        b.send(this.url);
    };
...
var SpringHotRoundtrip = (new function() {
    ...
    this.load = function() {
        ...
        if (this.offsetToday > 0) {
            var offsetToday = this.offsetToday, min = 3;
            if (offsetToday > 360) {
                min += (offsetToday - 360);
            }
        ...
        var sr = new ScriptRequest({ funcName: "SpringHotRoundtrip.parsedata", callbackName: "callback" });
        sr.send(_URL);
    };
    ...
    this.updateSevenDayToday = function(price) {
        ...
        if (this.cacheData && this.cacheData.out && this.cacheData.out[this._queryStr]) {
            ...
            this.cacheData = { out: {} };
            this.cacheData.out[this._queryStr] = { pr: this.nowprice, dt: $jex.date.format(this.searchDate) };
        }
    };
    this.sevenday = function() {
        ...
        this.eachDay(function(day) {
            ...
            _data[_dstr] = { date: _dstr, price: getDatePrice(_dstr) };
        ...
        $jex.$("searchDateBar").innerHTML = this.template_sevenday({ days: _data, currentDate: $jex.date.format(this.searchDate), _MODIFIERS: { GetTitle: function(date) { return date.replace(/.*\d{4}-(.*)/, "$1") + " " + ["周日", "周一", "周二", "周三", "周四", "周五", "周六"][$jex.date.parse(date).getDay()]; } } });
        var ul = $jex.$("searchDateBar").getElementsByTagName("ul")[0];
        if (ul) {
            var lis = ul.getElementsByTagName("li");
            $jex.array.each(lis, function(li, i) {
                if ($jex.ie == 6) {
                    $jex.hover({ act: li, onmouseover: function() { $jex.addClassName(li, "hover"); }, onmouseout: function() { $jex.removeClassName(li, "hover"); } });
                }
                if (i >= 7) {
                    return;
                }
    ...
    this.roundtripVendor = function(outLp, returndata, packagedata) {
        ...
        _keys.sort(function(a, b) { return self._packageData[a].packagePrice - self._packageData[b].packagePrice; });
        if (_keys.length < 1) {
            return;
        }
        var _templateContext = { keys: _keys, extData: this._packageData };
    ...
    this.springRoundRecommend_Load = function() {
        ...
        this.eachDay(function(day) {
            ...
            var size = 0;
            $jex.hash.each(_data, function() { size++; });
            return !(size >= _record);
        }, new Date(self.searchDate.getTime() + 1 * 24 * 3600000));
        var size = 0;
        $jex.hash.each(_data, function() { size++; });
        $jex.$("dSpringPanel").innerHTML = this.template_returnsuggestion({ data: _data, inSpringtrip: false });
        if (size > 0) {
    ...
    this.getPriceData = function(_dateStr) {
        ...
        var sr = new ScriptRequest({ funcName: "SpringHotRoundtrip.parsePriceData", callbackName: "callback" });
        sr.send(_URL);
    };
    ...
    this.priceCalendar = function() {
        ...
        var _templateContext = { nextMonth: _nextMonth, prevMonth: _prevMonth, dateArr: _dateArr, prevMonthDate: _firstDayOfLastMonthStr, nextMonthDate: _firstDayOfNextMonthStr, date: new Date(_year, _month - 1, 1), citystr: this.dc + "-" + this.ac, currentDate: $jex.date.format(this.searchDate) };
    ...
    this.patch = function(data) {
        ...
        var one_way = function(d, out) {
            $jex.hash.each(d, function(k, v) {
                ...
                $jex.merge(v, { url: out ? self.createCanlederUrl(v.dt) : self.createUrl(v.dt, false), price: v.pr, discount: v.dis, fromTime: v.dt, from: c[0], to: c[1] });
            });
        };
        one_way(data.re, false);
        one_way(data.out, true);
        $jex.hash.each(data.packagelist.normal, function(k, v) { v.price = v.pr; });
        if (this.nowprice && data.out[this._queryStr]) {
            data.out[this._queryStr].pr = this.nowprice;
        } else {
            if (this.nowprice && !data.out[this._queryStr]) {
                data.out[this._queryStr] = { pr: this.nowprice, dt: $jex.date.format(this.searchDate), url: this.createCanlederUrl($jex.date.format(this.searchDate)) };
            }
        }
        return data;
    };
...
var BookingPriceCheck = (function() {
    ...
    a.init = function() {
        ...
        PAGE_EVENT.bind("wrapper_price_change", function(d, c) { HoldLastShowFlight.goHoldUrl(d, c); });
    };
    a.check = function(j, h) {
        ...
        var g = c.getWrapperListType(), k = j.dataSource().type;
        var l = c.key() + "_" + g, n = j.wrapperId() + "_" + k;
        var d = PriceCheckService.getPriceInfo(l, n);
        if (d) {
            h = h || 0;
            var b = h == 1 ? d.pr : d.bpr, m = h == 1 ? j.afeePrice() : j.bprPrice();
            if (b !== m) {
...
var dflightTool = new function() {
    this.initialize = function(args) {
        ...
        var analyzer = DomesticOnewayDataAnalyzer;
        var param = window.location.param();
        this.args = args = $jex.merge({ fromCity: param.searchDepartureAirport, toCity: param.searchArrivalAirport, startDate: param.searchDepartureTime, alllowestprice: analyzer.lowestPrice(), DATA_RECOMMEND_AIRLINE: "http://ws.qunar.com/recommendAirline.jcp", DATA_RECOMMEND_AIRLINE_SINGLE: "http://ws.qunar.com/recommend" }, args || {});
    ...
    this.start = function(args) {
        ...
        var sr = new ScriptRequest({ funcName: "dflightTool.update", callbackName: "callback" });
    ...
    this.startSpecial = function(args) {
        ...
        var _startDate = this.startDate;
        var _params = "from=" + _fromCity + "&to=" + _toCity + "&start_date=" + _startDate + "&version=" + Math.random();
        var sr_single = new ScriptRequest({ funcName: "dflightTool.special", callbackName: "callback" });
    ...
    this.update = function(data) {
        ...
        if (data.records.length > 0) {
            ...
            $jex.$("dFlightPanel").innerHTML = this.template_flighttool({ data: _mydata });
    ...
    this.special = function(data) {
        ...
        var self = this, _d = [].concat(data.records), dc = this.fromCity, ac = this.toCity;
        var f = function(o1) {
        ...
        _d = _d.sort(function(a, b) { return f(a) - f(b); });
        this._cacheD = _d;
    ...
    this.updateSpecial = function(alllp) {
        ...
        $jex.$("dflightRecommendPanel").innerHTML = this.template_dflight_recommendtools({ now: lp, dc: dc, ac: ac, sub: alllp - lp.pr });
...
var Trendflash = {
    _cache: {}, _cacheLength: 0, _controls: null,
    init: function(c) {
        var a = DomesticOnewaySearchService;
        var e = DomesticOnewayDataAnalyzer;
        this.args = c = { hasOneWay: (e.lowestOneway() != null), isValidQuery: a.isValidQuery(), isInter: false, dc: window.location.param().searchDepartureAirport, ac: window.location.param().searchArrivalAirport, title_id: "trendTitle", flash_id: "hdivTrendFlash", flash_panel: "dTrendflashPanel", MandatoryInsurance: {} };
        ...
        setTimeout(function() { Trendflash.add(b, d, { displayDelBtn: false, isFirstLoad: true }); }, 0);
    ...
    add: function(c, e, b) {
        ...
        Trendflash._cache[f] = { lineid: a };
        Trendflash._cacheLength += 1;
        if (!b.isFirstLoad) {
            window.addData = function() { return "http://ws.qunar.com/holidayService.jcp?lane=" + encodeURIComponent(c) + "-" + encodeURIComponent(e); };
            thisMovie("main").add();
        }
    }
};

function getData() { return "http://ws.qunar.com/holidayService.jcp?lane=" + encodeURIComponent(Trendflash.args.dc) + "-" + encodeURIComponent(Trendflash.args.ac); }
...
ConfigManager.setConfig({ "default": { acf: 0, fot: 0, safePrice: Number.MAX_VALUE, carrier: { full: "", key: "", zh: "" }, plane: { full: "未知机型", key: "NULL", type: [] }, airport: { ab: "", code: "", full: "" }, city: { codeList: "", en: "", zh: "" } }, config: { timerange: { "0": { zh: "上午", value: 0 }, "1": { zh: "中午", value: 1 }, "2": { zh: "下午", value: 2 }, "3": { zh: "晚上", value: 3 } }, services: { s1: "CATA认证", s2: "7×24服务", s3: "去哪儿帮您填", s4: "支付价出票", s5: "退改签保障", s6: "赠送保险", s7: "服务保障", s8: "免费邮寄", s9: "担保通" }, servicesDesc: { s1: "获得《中国民用航空运输销售代理业务资格认可证书》", s2: "提供7×24小时服务", s3: "支持使用在去哪儿网填写的乘机人信息", s4: "承诺按照支付价格出票", s5: "严格执行航空公司退改签规定", s6: "承诺购买机票赠送保险", s7: "服务保障", s8: "购买机票可免费邮寄行程单", s9: "全程预订保障，去哪儿都放心" } } });
...
flightResultController.prototype.initUI = function() {
    ...
    this.resultList = new FlightListUI({ elemId: "hdivResultPanel" });
    this.filterGroup = new DomesticOnewayFilterListUI({
        elemId: "hdivfilterPanel", filterConf: { "起飞机场": { type: 8 }, "降落机场": { type: 8 }, "起飞时间": { sort: { "上午": 0, "中午": 1, "下午": 2, "晚上": 3 } }, "机型": { sort: { "大型机": 0, "中型机": 1, "小型机": 2, "麦道系列": 3, "其他机型": 4, "未知机型": 5 }, type: 8 } },
        on: {
            ...
            openMore: function() { b.trackFilters(true); }
        }
    });
    ...
    this.pagesizer = pagesizer = new XSelect({
        elemId: "hdivPageSizer", initFire: false, values: [{ value: 30, name: "30" }, { value: 60, name: "60" }],
        on: {
    ...
    $jex.event.binding(this.service, "TransferDataReady", function() { b.filterGroup.setTransformLoad(true); });
    ...
    this.sort_time_handler = new SortHandler({
        elemId: "btnDepttimeOrderArror", sortKey: "deptTimeValue",
        on: {
    ...
    this.sort_price_handler = new SortHandler({
        elemId: "btnPriceOrderArror", sortKey: "lowestPrice",
        on: {
            clickSort: function(d) {
                var c;
                if (d[0][1]) {
    ...
    this.statusbar = new OneWaySearchStatusbar({ elemId: "progTip", service: b.service, analyzer: b.analyzer });
...
flightResultController.prototype.bindUI = function() {
    ...
    $jex.event.binding(b.analyzer, "updateFilter", function(h) { f.addFilter(h); });
    ...
    TsinghuaOneWayTracker.clearTimerToSaveTrack = function() { clearTimeout(c); };
...
(function() {
    ...
    $jex.foreach(f, function(s, e, r) { $jex.console.trace("[PARAM]", r, ":", s); });
    $jex.console.trace("[CLIENT]", window.navigator.userAgent.toString());
    window.location.param = function() { return f; };
    window.location.research = function(w, u, s, e, v) {
        var t = $jex.merge(f, { searchDepartureAirport: w || f.searchDepartureAirport, searchArrivalAirport: u || f.searchArrivalAirport, searchDepartureTime: s || f.searchDepartureTime, searchArrivalTime: e || f.searchArrivalTime, from: v || f.from });
    ...
    window.System = { service: DomesticOnewaySearchService, analyzer: DomesticOnewayDataAnalyzer, param: f };
    ...
    var l = new FlashAdUI({ elemId: "hdivResultPanel" });
    var b = function() {
    ...
    var c = new CACTI_monitoring({ url: "http://bmrg.qunar.com/f", pageId: "DomesticOnewayList", timerList: ["t_done", "t_firstData"] });
    var k = 0;
    ...
    $jex.event.bind(window, "load", function() { q(); });
    var o = false;
    var m = function() {
        if (o) {
            return;
        ...
        setTimeout(function() {
            function r() { setTimeout(function() { SpringHotRoundtrip.updateSevenDayToday(); }, 0); }

            $jex.console.start("第一屏,七日低价 ");
            SpringHotRoundtrip.initialize({ dc: f.searchDepartureAirport, ac: f.searchArrivalAirport, searchDate: $jex.date.parse(f.searchDepartureTime), config: oneway_config.SpringHotConfig, isInter: false });
            SpringHotRoundtrip.load();
            $jex.event.binding(h, "dataComplete", function() { r(); });
            $jex.event.binding(PAGE_EVENT, "lowPriceChange", function() { r(); });
    ...
    function p() {
        ...
        $jex.event.binding(i, "invalidQuery", function() { TraceAnalyzer.all.invalidErr(); });
        $jex.event.binding(i, "noTransferData", function() { TraceAnalyzer.all.noTransErr(); });
        ...
        $jex.event.binding(i, "loadedFirstData", function() { m(); });
    ...
    $OTA.group.options({ type: "ow", currentDate: window.SERVER_TIME || new Date(), fromCity: f.searchDepartureAirport, toCity: f.searchArrivalAirport, fromDate: new Date(f.searchDepartureTime.replace(/-/g, "/")) });
    $jex.event.binding(i, "loadedLongwell", function(e) { $OTA.group.options({ queryID: e.queryID }); });
    $jex.event.binding(i, "loadedOnewayData", function(e) { $OTA.extract(e); });
    TraceAnalyzer.all = TraceAnalyzer.create().queryInfo({ da: f.searchDepartureAirport, aa: f.searchArrivalAirport, inter: 0, dd: f.searchDepartureTime, now: $jex.date.format(SERVER_TIME), ip: CLIENT_IP, act: "noresult" });
    HotSale.init();
    p();
...
function FEMonitor(a) {
    a = $jex.merge({ logurl: "http://femon.qunar.com/felog", interval: 0, module: "F" }, a);
    this._init(a);
}
...
FEMonitor.fn.addMonitor = function(e, c, d) {
    ...
    var a = this, b = function(f) { a._sendLog(d || f); };
...
FEMonitor.fn._sendLog = function(c) {
    ...
    var d = this.module + "_" + c, b = new Date().getTime(), a = { id: d, n: 1, type: 1, s: this.interval, t: b, token: this._calcToken(d, b) };
...
FEMonitor.fn._canSendLog = function() { return this.interval === 0 ? true : new Date().getTime() - this.lastSendTime > this.interval * 1000; };
...
FEMonitor.fn._calcToken = function(b, a) { return ""; };
var fem = new FEMonitor({ module: "F_LP_FL_OW" });
var __$__ = $jex.$, listPanel = __$__("hdivResultPanel");
...
(function() {
    ...
    window.searchCaution = function() {
        ...
        var i = { "国内_oneway": "fi_dom_search", "国内_roundtrip": "fi_ont_search", "国际_oneway": "fi_int_search", "国际_roundtrip": "fi_ont_search", "特价_lowestprice": "tejia_fi" };
        var q = { oneway: "OnewayFlight", roundtrip: "RoundTripFlight", lowestprice: "DealsFlight" };
